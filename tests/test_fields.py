...
def test_voxelFields_grid_staggered_x():
    Nx, Ny, Nz = 10, 5, 7
    vf = vox.VoxelFields((Nx + 1, Ny, Nz), (Nx, Ny, Nz), convention='staggered_x')
    (x,y,z) = vf.meshgrid()
...
def test_sphere_volume_fraction():
    vf = vox.VoxelFields((6, 5, 5), convention='staggered_x')
    vf.add_field("sphere")
