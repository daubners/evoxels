...
from .solvers_basic import BaseSolver
...
class PeriodicCahnHilliardSolver(BaseSolver):
    ...
    def __init__(self, data: VoxelFields, fieldname, diffusivity=1.0, epsilon=3, rhs_in=None, device='cuda'):
        """
        ...
        if rhs_in:
            self.data.add_grid()
            self.grid = torch.stack((torch.from_numpy(self.data.grid[0]), \
                                     torch.from_numpy(self.data.grid[1]), \
                                     torch.from_numpy(self.data.grid[2])), \
                                     dim=0).to(dtype=self.precision, device=self.device)
    ...
    def solve(self, time_increment=0.1, frames=10, max_iters=100, \
              method='mixed_FFT', A = 0.25, \
              verbose=True, vtk_out=True, plot_bounds=None):
        """
        ...
        if self.data.convention != 'cell_center':
            raise ValueError("The fully periodic solver assumes cell centered grid convention.")
        ...
        with torch.no_grad():
            ...
            for i in range(max_iters):
                ...
                if method == 'explicit':
                    ...
                    mu = 18/self.eps*self.field*(1-self.field)*(1-2*self.field) \
                        - 2*self.eps*laplace
                    self.apply_periodic_BC_cell_center(mu)
                    divergence = self.calc_divergence_variable_mobility(mu, self.field)
                    self.field[:,1:-1,1:-1,1:-1] += time_increment * self.D * divergence
                    if self.rhs:
                        self.field[:,1:-1,1:-1,1:-1] += time_increment * self.rhs(time_increment*i, *self.grid)

                elif method == 'full_FFT':
                    c_inner = self.field[:,1:-1,1:-1,1:-1]
                    dfhom_dc = 18/self.eps*c_inner*(1-c_inner)*(1-2*c_inner)
                    ...
                    if self.rhs:
                        ...
                        c_hat += time_increment*(self.D*flux_div + rhs_term) / (1 + 2*self.eps*time_increment*self.D*k_squared**2*A)
                    else:
                    ...
                    self.apply_periodic_BC_cell_center(self.field)
                    laplace = self.calc_laplace(self.field)
                    mu = 18/self.eps*self.field*(1-self.field)*(1-2*self.field) - 2*self.eps*laplace
                    self.apply_periodic_BC_cell_center(mu)
                    divergence = self.calc_divergence_variable_mobility(mu, self.field)
                    divergence *= self.D
                    if self.rhs:
                    ...
                    c_hat = fft.fftn(self.field[:,1:-1,1:-1,1:-1])
    ...
    def handle_outputs(self, vtk_out, verbose, slice=0, iter=None):
        if self.data.convention == 'staggered_x':
            self.data.fields[self.fieldname] = self.field[:,:,1:-1,1:-1].squeeze(0).cpu().numpy()
        else:
            self.data.fields[self.fieldname] = self.field[:,1:-1,1:-1,1:-1].squeeze(0).cpu().numpy()
        if np.isnan(self.data.fields[self.fieldname]).any():
            if iter:
                print(f"NaN detected in frame {self.frame} at time {self.time} in iteration {iter}. Aborting simulation.")
            else:
        ...
        if verbose == 'plot':
            ...
            self.data.plot_slice(self.fieldname, slice, time=self.time, value_bounds=self.plot_bounds)
...
class MixedCahnHilliardSolver(PeriodicCahnHilliardSolver):
    ...
    Attributes:
        ...
        device: device for storing torch tensors
    """
    def __init__(self, data: VoxelFields, fieldname, diffusivity=1.0, epsilon=3, \
                 mode='dirichlet', rhs_in=None, device='cuda'):
        super().__init__(data, fieldname, diffusivity=diffusivity, epsilon=epsilon, \
                         rhs_in=rhs_in, device=device)
        self.mode = mode
    ...
    def flip_zero_dirichlet_periodic_staggered_x(self, tensor):
        zeros = torch.zeros(tensor.shape[0], 1, tensor.shape[2], tensor.shape[3], device=self.device)
        pad_right = -tensor[:, :, :, :]
    ...
    def initialise_FFT_wavenumbers(self, convention):
        ...
        if convention == 'staggered_x':
            kx = 2 * torch.pi * fft.fftfreq(2*self.data.Nx-2, d=dx, device=self.device)
        else:   
    ...
    def solve(self, time_increment=0.1, frames=10, max_iters=100, \
              method='mixed_FFT', A = 0.25, bcs = (0.0,0.0), \
              verbose=True, vtk_out=True, plot_bounds=None):
        """
        Solves the Cahn-Hilliard equation using specified timestepping method:
         - 'explicit': explicit Euler
        ...
        with torch.no_grad():
            ...
            for i in range(max_iters):
                ...
                mu = 18/self.eps*self.field*(1-self.field)*(1-2*self.field) - 2*self.eps*laplace
                self.apply_mu_bc(mu)
                divergence = self.D * self.calc_divergence_variable_mobility(mu, self.field)
                if self.rhs:
                    divergence += self.rhs(time_increment*i, *self.grid)
                ...
                if method == 'explicit':
                    ...
                    dc_fft = time_increment*div_fft / (1 + 2*self.eps*time_increment*self.D*k_squared**2*A)
                    if self.data.convention == 'cell_center':
                        self.field[:,1:-1,1:-1,1:-1] += torch.real(fft.ifftn(dc_fft))[:,:self.data.Nx,:,:]
                    else: 
                        self.field[:,:,1:-1,1:-1] += torch.real(fft.ifftn(dc_fft))[:,:self.data.Nx,:,:]
